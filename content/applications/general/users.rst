...
:guilabel:`User Type` can be chosen on the :guilabel:`Manage Users` page clicking on the
:guilabel:`search` bar, and then setting a filter for either :guilabel:`Internal User` or
:guilabel:`Portal User`. Once that filter is set, click :guilabel:`New` in the upper-left corner to
create a user account with that setting.
